"""
train_hipxgen_symp.py — Training script for HipXGen-Symp.

Uses sequential cross-attention (x2) with RecA backbone and symptom-only text.
With weaker symptom-only text, image guidance plays a larger role in classification.

Usage:
    python train_hipxgen_symp.py
    python train_hipxgen_symp.py --smoke-test
"""

import os
os.environ.setdefault("TEXT_JSON",
    os.path.expanduser("~/data/patient_text_symptom_only.json"))
...
from model_hipxgen_symp import HipXGenSymp
                                       # but load RecA backbone via env RECA_CKPT

CHECKPOINT_DIR  = os.environ.get("CHECKPOINT_DIR", os.path.join(os.path.dirname(__file__), "checkpoints"))
LOG_CSV         = os.environ.get("LOG_CSV",        os.path.join(os.path.dirname(__file__), "training_log_symp.csv"))
BEST_CKPT       = os.path.join(CHECKPOINT_DIR, "best_model_symp.pt")
...
def run_epoch(model, loader, criterion, optimizer, device, smoke_test=False):
    ...
    for batch_idx, (imgs, ids, masks, ops, labels) in enumerate(
        ...
        logits, attn1, attn2 = model(imgs, ids, masks, ops)
        cls_loss = criterion(logits, labels)
        ent_loss = model.attention_entropy_loss(attn1) + model.attention_entropy_loss(attn2)
...
def evaluate(model, loader, criterion, device, smoke_test=False):
    ...
    for batch_idx, (imgs, ids, masks, ops, labels) in enumerate(
        ...
        logits, _, _ = model(imgs, ids, masks, ops)
...
def run_test(model, device):
    ...
    for imgs, ids, masks, ops, labels in loader:
        logits, _, _ = model(imgs.to(device), ids.to(device),
                             masks.to(device), ops.to(device))
...
def main(smoke_test=False):
    ...
    print(f"Image  : RecA backbone (frozen)")
    print(f"Text   : SmolLM2-1.7B (frozen) + symptom-only text")
    print(f"Fusion : Sequential GuidedCrossAttention x2 (image-guided)")
    print(f"Loss   : CrossEntropy + {ENT_LOSS_WEIGHT}×EntropyLoss x2")
    ...
    # Load HipXGen-Symp architecture with RecA backbone
    reca_ckpt = os.path.join(CHECKPOINT_DIR, "reca_backbone.pt")
    model = HipXGenSymp(mae_ckpt=reca_ckpt).to(device)
