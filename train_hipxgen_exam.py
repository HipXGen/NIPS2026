"""
train_hipxgen_exam.py — Training script for HipXGen-Exam.

Usage:
    python train_hipxgen_exam.py
    python train_hipxgen_exam.py --smoke-test
"""

import os
os.environ.setdefault("TEXT_JSON",
    os.path.expanduser("~/data/patient_text_no_dx.json"))
...
LOG_CSV         = os.environ.get("LOG_CSV",        os.path.join(os.path.dirname(__file__), "training_log_exam.csv"))
BEST_CKPT       = os.path.join(CHECKPOINT_DIR, "best_model_exam.pt")
...
LABEL_SMOOTHING = 0.1
GRAD_CLIP       = 1.0
ENT_LOSS_WEIGHT = 0.05   # smaller weight — only one attn map
...
def main(smoke_test=False):
    ...
    print(f"Image  : RecA-pretrained backbone (frozen)")
    ...
    print(f"Loss   : CrossEntropy + {ENT_LOSS_WEIGHT}×EntropyLoss (1 attn map)")
    print(f"Train  : pre_op + not_applicable only (same dist as test)")
    print(f"Sampler: WeightedRandomSampler")
    print(f"Stop   : weighted val F1, patience={PATIENCE}, max={MAX_EPOCHS} epochs")
